...
contract EchoCC is ReentrancyGuard, Admin, Handler, Banker, Signer, CrossChainSupport {
    event TradeExecuted(string id);

    constructor(address owner, address signer, address wormhole, uint16 chainId, uint8 wormholeFinality)
        Admin(owner, signer)
        CrossChainSupport(wormhole, chainId, wormholeFinality)
    {}
